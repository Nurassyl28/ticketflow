...
| `auth_sessions` | SHA-256 токена, пользователь, время создания и истечения сессии |
...
| Сессии | При удалении пользователя его сессии удаляются каскадно; срок и SHA-256 проверяются ограничениями БД |
...
Частичные индексы не проверяют текущее время. Сервис бронирования под блокировкой
события и `event_seats` переводит просроченную бронь в `EXPIRED`, проверяет существующий
билет и резервирует все запрошенные места одной транзакцией.
...
бронирования с продажей реализовано общей блокировкой события и транзакциями.
Проверка ролей и владельца реализована в [слое доступа](auth.md). Вместимость,
сумма заказа, оплата перед выпуском и переходы статусов проверяются сервисами
каталога, бронирования, оплаты и отмен.
...
Миграция `0002` добавляет только `auth_sessions` и её индексы; существующие
пользователи, каталог и покупки сохраняются. Откат `0002` завершает все сессии.
